...
namespace FileArchiver.Services.Services.Implementation
{
    public class UserService : IUsersService
    {
        ...
        public FileViewModel GetUserByFileId(int fileId)
        {
            FileViewModel result = new FileViewModel();
            var user = _filesRepository.GetUserByFileId(fileId);
            if (user == null)
                return result;
            result.FileId = user.Id;
            result.CreatorUsername = user.User.Name;
            return result;
        }
        public UserViewModel GetUserIdByUsername(string username)
        {
            UserViewModel result = new UserViewModel();
            var user = _userRepo.GetUserIdByUsername(username);
            if (user == null)
                return result;
            result.Id = user.Id;
            return result;
