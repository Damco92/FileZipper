...
namespace FileArchiverCommmon.Helpers
{
    public static class HashingHelper
    {
       ...
       public static bool IsTextFile(string fileName)
        ...
        public static bool CheckMaskAccordingToFileName(string fileMime, string fileName)
        {
            bool result;
            switch (fileMime)
            {
                ...
                case "application-pdf":
                 result = IsPdfFile(fileName);
                    break;
                case "image-png":
                    result = IsImageFile(fileName);
                    break;
                case "application-json":
                    result = IsJsonFile(fileName);
                    break;
                case "image-jpeg":
                    result = IsJPGFile(fileName);
                    break;
                case "application-xml":
                    result =  IsXMLFile(fileName);
                    break;
                case "application-zip":
                    result = IsZipFile(fileName);
                    break;
                case "application-x-7z-compressed":
                    result =  Is7ZipFile(fileName);
                    break;
                default:
                    result = false;
                    break;
